...
class CartFormula7F4 extends StatelessWidget {

  final TableF4 data;

  final TableF3? tableF3;

  const CartFormula7F4({super.key,required this.data,this.tableF3});

  @override
  Widget build(BuildContext context) {
    String TextFormula = '( F3_β - F4_β ) / min';
    ...
    if(tableF3!=null) {
      ...
      double min = Math.min<double>(tableF3!.Beta_Ec!, data.Beta_Ec!);

      ResultFormula = '(${tableF3!.Beta_Ec!} - ${data.Beta_Ec!}) / $min';

      Res = ((tableF3!.Beta_Ec! - data.Beta_Ec!) / min) * 100;
